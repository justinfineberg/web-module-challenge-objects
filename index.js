...
const eggs = createMenuItem('eggs', 13, 'Breakfast')
const steak = createMenuItem('Steak', 54, 'Dinner')
console.log(pizza)
console.log(eggs)
console.log(steak)
...
function getReviewByRating(arr, number) {
  let answerArr = [];
 for (let i=0; i < arr.length; i++){
   let rating = arr[i]['rating']
   if (rating >= number && rating <= number+.9){
...
function getLongReviews(arr) {
  let longArr = [];
  for (let i=0; i< arr.length; i++){
    let str = arr[i]['feedback']
    let feedbackLength = str.split(' ').length
